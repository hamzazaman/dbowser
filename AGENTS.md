...
- The current implementation uses a small asyncpg pool shared across requests; keep it read-only with `default_transaction_read_only` and timeouts as already configured.
...
- User config lives under `~/.config/.dbowser` and includes `connections.json`, `query.sql`, and `last_selection.json`.
